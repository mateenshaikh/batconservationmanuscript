...
The data is a combination of data from various sources, such as IUCN, with non-distribute licenses. The data can be freely downloaded and used but not distributed, such as on github. 
...
The area being such large numbers overwhelms other smaller numbers in some calculations due to rounding issues, so we  convert to `Area.mkm` (area in mega meters or million sq km instead of just Area in sq km.)
```{r}
name.data=data.frame(name.data,Area.mkm=name.data$Area.km/10^6)
```

### Capitalize some variables for presentability
...
### Collapse/combine some habitats
...
#code for windows  parallelization (works on mac/linux too)
library(parallel)
...
imputedModelsGLS          = parLapplyLB(cl = cl, fun=fitFullModel.collapsedhabitats, X=new.traits.list )
imputedModelsGLS.complete = parLapplyLB(cl = cl, fun=fitFullModel.collapsedhabitats, X=new.traits.list.complete)
...
### Helper variables used in analysis
...
completeCases = complete.cases(name.data)
 ...
 knownPredictedStatuses=   data.frame(
   actual    =actualStatus[knownSpecies],
   predicted =averagePredictedStatus.imputedgls[knownSpecies],mod="gls",dataset="imputed"
  )
...
#put all the estimates into a dataframe
...
Manually calculate the standard errors and corresponding quantities make confidence intervals and pvalues
```{r,class.source='fold-hide'}
...
#for  α=.05 find the bounds (lb,ub) and then the teststat-> pvalue
siglevel = 0.05; crit = -qnorm(siglevel/2)
...
```{r}
#print the pvalues for the estimates in a nice table for html
kable(data.frame(avgEstimate,stdpooled,pval,flags))
```
## for intervals
```{r}
#print the conf intervals for the estimates in a nice table for html
...
## Note about the reference variable
...
ggplot(subsettable,aes(x=Min.temp.coldest.month,y=Conservation.category.numeric,group=Conservation.category.numeric))+
  geom_boxplot()+xlab("Coldest temp (°C)")+ylab("Known Conservation Category")
...
## Create an approximate correlation between fitted and actual values (like R^2)
